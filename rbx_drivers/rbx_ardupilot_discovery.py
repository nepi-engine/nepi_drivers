...
class ArdupilotDiscovery:
  ...
  def discoveryFunction(self,available_paths_list, active_paths_list,base_namespace,nex_dict = TEST_NEX_DICT):
    ...
    if self.system_type == 'Real':
      ...
      for path_str in self.path_list:
        ...
        self.path_list = self.sitl_addr_list
        for path_str in self.path_list:
          valid_path = True
          for id in self.ignore_id_list:
            if path_str.find(id) != -1 or path_str in self.active_paths_list:
  ...
  def launchRealDeviceNode(self, path_str):
    ...
    if self.comp_id != None and self.sys_id_mavlink != None and self.addr_str != None:
          path_str_short = path_str.split('/')[-1]
          mavlink_node_name = "mavlink_" + path_str_short
          ...
          ardu_node_name = "ardupilot_" + path_str_short
          ...
          fgps_node_name = "fake_gps_" + path_str_short
          ...
          try:
            ...
            device_entry = dict()
            device_entry["sysid"] = sys_id_mavlink
            device_entry["compid"] =  comp_id   
            ...
            active_devices_dict[path_str] = device_entry
            success = True
          except:
            rospy.logerr(self.log_name + ": Failed to start " + mavlink_node_name)
    return success
