...
class KistKPT20Discovery:
    ...
    ########################
      ...
      if self.connection == 'TCP-Server':
        try:
            ...
            self.logger.log_warn("Not a valid IP address " + ip_addr + " " + str(e))#
            return self.active_paths_list
        try:
            test = int(ip_port)
        except Exception as e:
            self.logger.log_warn("Not a valid IP port " + ip_port + " " + str(e))#
            return self.active_paths_list
    ...
    for path_str in self.path_list:
      ...
      for id in self.excludedDevices:
        if path_str.find(id) != -1 or path_str in self.active_paths_list:
          valid_path = False
      if valid_path:
        #self.logger.log_info("Looking for path: " + path_str)
        #self.logger.log_info("In path_list: " + str(self.active_paths_list))
  ...
  def checkForDevice(self,path_str):
    #self.logger.log_warn("Sealight checkForDevice start")###
    found_device = False
        ...
        for addr_str in self.addr_search_list:
          ...
          if len(response) > 5:
            if response[0:5] == ser_msg[0:5]:
              ...
              self.logger.log_info("Found device at baudrate: " + baud_str)
